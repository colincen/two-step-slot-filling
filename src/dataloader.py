...
class Dataset(data.Dataset):
    def __init__(self, X, y0, y1, y2, domains, template_list=None):
        self.X = X
        self.y0 = y0
        self.y1 = y1
        self.y2 = y2
    ...
    def __getitem__(self, index):
        if self.template_list is not None:
            return self.X[index], self.y0[index], self.y1[index], self.y2[index], self.domains[index], self.template_list[index]
        else:
            return self.X[index], self.y0[index], self.y1[index], self.y2[index], self.domains[index]
    
    def __len__(self):
        return len(self.X)

def collate_fn_for_label_encoder(data):
    X, y0, y1, y2, domains, templates = zip(*data)
    ...
    return padded_seqs, lengths, y0, y1, y2, domains, padded_templates, tem_lengths

def collate_fn(data):
    X, y0, y1, y2, domains = zip(*data)
    lengths = [len(bs_x) for bs_x in X]
    max_lengths = max(lengths)
    padded_seqs = torch.LongTensor(len(X), max_lengths).fill_(PAD_INDEX)
    for i, seq in enumerate(X):
    ...
    return padded_seqs, lengths, y0, y1, y2, domains

def get_dataloader(tgt_domain, batch_size, use_label_encoder, n_samples):
    
    all_data, vocab = datareader(use_label_encoder)
    if use_label_encoder:
        train_data = {"utter": [],"y0":[], "y1": [], "y2": [], "domains": [], "template_list": []}
    else:
        train_data = {"utter": [], "y0":[], "y1": [], "y2": [], "domains": []}
    ...
    if use_label_encoder:
        dataset_tr = Dataset(train_data["utter"], train_data["y0"], train_data["y1"], train_data["y2"], train_data["domains"], train_data["template_list"])
    else:
        dataset_tr = Dataset(train_data["utter"], train_data["y0"], train_data["y1"], train_data["y2"], train_data["domains"])
    dataset_val = Dataset(val_data["utter"], val_data["y0"],val_data["y1"], val_data["y2"], val_data["domains"])
    dataset_test = Dataset(test_data["utter"], test_data["y0"], test_data["y1"], test_data["y2"], test_data["domains"])
